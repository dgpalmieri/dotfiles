...
`);
  return new Promise(function(resolve) {
    import_child_process.default.execFile(doqPath, [...args, "-f", tmpFile.name], opts, function(err) {
      if (err) {
...
var PydocstringCodeActionProvider = class {
  async provideCodeActions(document, range, context) {
    ...
    if (range.start.line === range.end.line && range.start.character === 0) {
      ...
      const resolveRange = import_coc2.Range.create({character: range.start.character, line: range.start.line}, {character: range.end.character, line: endBlockRangeLine});
    ...
    if (range.start.line < range.end.line && !this.wholeRange(doc, range)) {
      ...
      for (let i = endBlockRangeLine; i <= fullRangeLength; i++) {
        if (doc.getline(i + 1).trim() !== "" && !doc.getline(i + 1).trim().startsWith("#")) {
...
async function activate(context) {
  ...
  context.subscriptions.push(import_coc4.commands.registerCommand("pydocstring.runAction", async (document, range) => {
    ...
    if (doc.getline(range.start.line).match(/\s*class\s*.*/) || doc.getline(range.start.line).match(/\s*def\s*.*/)) {
      endCharacter = 0;
      endLine = range.end.line + 1;
    }
    range = import_coc4.Range.create({line: range.start.line, character: range.start.character - startLineIndentLength}, {line: endLine, character: endCharacter});
  ...
  const actionProvider = new action_default();
